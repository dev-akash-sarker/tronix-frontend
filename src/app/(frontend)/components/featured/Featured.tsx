  ...
  useEffect(() => {
    const fetchFeatured = async () => {
      try {
        ...
        // FIX: response is an array, not an object
        const featured = res.data[0];

        // Extract product IDs
        const ids = featured.products.map((p: any) => p._id);

        // STEP 2: Fetch each product
                  ...
                  <div className="bg-white w-[312px] mx-auto">
                    ...
                    <div className=" font-pop text-2xl flex justify-center items-center gap-x-2">
                      ...
                      <span className=" font-bold text-hover-social">
                        {product.persentage
                          ? updatedPrice(product.price, product.discountPercentage)
